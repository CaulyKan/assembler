...
use crate::InstructionPointerValidity::InstructionPointerValidity;
...
pub struct InstructionStream<'a> {
    ...
    instruction_pointers_to_replace_labels_with_8_bit_displacements:
        Vec<(Label, InstructionPointer)>,
    instruction_pointers_to_replace_labels_with_32_bit_displacements:
        Vec<(Label, InstructionPointer)>,
...
impl<'a> InstructionStream<'a> {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            instruction_pointers_to_replace_labels_with_8_bit_displacements: Vec::with_capacity(
                instruction_stream_hints.number_of_8_bit_jumps,
            ),
            instruction_pointers_to_replace_labels_with_32_bit_displacements: Vec::with_capacity(
                instruction_stream_hints.number_of_32_bit_jumps,
            ),
    ...
    fn attempt_to_resize_in_place(&mut self) -> io::Result<()> {
        let new_length = self
            .executable_anonymous_memory_map
            .attempt_to_resize_in_place_whilst_writing()?;
    ...
    fn hints_for_next_instance(&self) -> InstructionStreamHints {
        InstructionStreamHints {
            ...
            number_of_8_bit_jumps: self
                .instruction_pointers_to_replace_labels_with_8_bit_displacements
                .len(),
            number_of_32_bit_jumps: self
                .instruction_pointers_to_replace_labels_with_32_bit_displacements
                .len(),
    ...
    pub fn finish(mut self) -> (&'a [u8], InstructionStreamHints) {
        ...
        for (label, insert_at_instruction_pointer) in self
            .instruction_pointers_to_replace_labels_with_8_bit_displacements
            .iter()
        {
            ...
            let result = self
                .byte_emitter
                .insert_8_bit_effective_address_displacement(
                    *insert_at_instruction_pointer,
            ...
            .iter()
        {
            ...
            let result = self
                .byte_emitter
                .insert_32_bit_effective_address_displacement(
                    *insert_at_instruction_pointer,
                    target_instruction_pointer,
                );
        ...
        for (label, insert_at_instruction_pointer) in self.emitted_labels.iter() {
            ...
            self.byte_emitter.emit_u64_at(
                target_instruction_pointer as u64,
                *insert_at_instruction_pointer,
            )
        }
        ...
        let slice =
            unsafe { from_raw_parts(self.start_instruction_pointer() as *const u8, length) };
        (slice, hints)
    }

    #[inline(always)]
    fn target_instruction_pointer(&self, label: Label) -> InstructionPointer {
        self.labelled_locations
            .potential_target_instruction_pointer(label)
    }
    ...
    fn valid_target_instruction_pointer(&self, label: Label) -> InstructionPointer {
        ...
        debug_assert!(
            target_instruction_pointer.is_valid(),
            "unresolved {:?}'",
            label
        );
        target_instruction_pointer
    ...
    pub fn rewind_to_emit_double_word(&mut self, double_word: u32) {
        let instruction_pointer = self.instruction_pointer();
        self.byte_emitter
            .emit_u32_at(double_word, instruction_pointer - 4)
    }
    ...
    fn vex_5(&mut self, mmmmm: u8, L: u8, pp: u8, W: u8, vvvv: impl Register) {
        if mmmmm == 0x01 && W == 0 {
            ...
            self.byte_emitter
                .emit_3_byte_vex_prefix(0x80, 0x40, 0x20, mmmmm, W, vvvv, L, pp)
        }
    }
    ...
    #[inline(always)]
    fn displacement_immediate_2(
        &mut self,
        displacement1: Immediate8Bit,
        displacement2: Immediate16Bit,
    ) {
    ...
    fn displacement_label_8bit(&mut self, label: Label) -> ShortJmpResult {
        ...
        if target_instruction_pointer.is_valid() {
            ...
            self.instruction_pointers_to_replace_labels_with_8_bit_displacements
                .push((label, insert_at_instruction_pointer));
            Ok(())
        }
    }
    ...
    fn displacement_label_32bit(&mut self, label: Label) {
        ...
        if target_instruction_pointer.is_valid() {
            self.byte_emitter
                ...
                .expect("32-bit JMP was too far")
        } else {
            self.instruction_pointers_to_replace_labels_with_32_bit_displacements
                .push((label, insert_at_instruction_pointer));
        }
    }
    ...
    #[inline(always)]
    fn relative_address_32bit(
        &self,
        absolute_address: impl FunctionPointer,
        offset_to_end_of_opcode_encoding: usize,
    ) -> RelativeAddress32Bit {
        RelativeAddress32Bit(
            ((absolute_address.absolute_virtual_address() as isize)
                - ((self.instruction_pointer() + offset_to_end_of_opcode_encoding) as isize))
                as i32,
        )
    }
    ...
    pub fn jmp_Any64BitMemory_statically_relative_address(
        ...
        let memory_destination = Any64BitMemory::index_64_scale_displacement(
            index_register,
            scale,
            ArtificallyLargeDisplacementPlaceholder,
        );
        ...
        let memory_destination = Any64BitMemory::base_64_index_64_scale_displacement(
            base_register_holding_start_of_instructions_pointer,
            index_register,
            scale,
            ArtificallyLargeDisplacementPlaceholder,
        );
        ...
        debug_assert!(
            displacement <= ::std::i32::MAX as usize,
            "Jumps of more than 2Gb are not supported"
        );
    ...
    pub fn statically_relative_address<BM: BitMemory>(
        ...
        BM::statically_relative_address(
            self,
            array_location_in_memory,
            index_register,
            scale,
            base_register_holding_start_of_instructions_pointer,
        )
    ...
    pub fn overwrite_last_32bit_displacement_with_relative_address_to(
        ...
        debug_assert!(
            location_in_memory <= ::std::isize::MAX as usize,
            "location_in_memory is larger than ::std::isize::MAX"
        );

        let instruction_pointer = self.instruction_pointer();
        debug_assert!(
            instruction_pointer <= ::std::isize::MAX as usize,
            "instruction_pointer is larger than ::std::isize::MAX"
        );

        let offset = (location_in_memory as isize) - (instruction_pointer as isize);
        debug_assert!(
            offset <= (::std::i32::MAX as isize) && offset >= (::std::i32::MIN as isize),
            "offset to location_in_memory is bigger than a 32-bit displacement can hold"
        );
    ...
    pub fn emit_fixed_size_block<R>(
        &mut self,
        scale: impl Into<u8>,
        mut emit_instructions: impl FnMut(&mut Self) -> R,
    ) -> R {
        ...
        debug_assert!(
            ...
            "block_size '{}' exceeds desired_block_size '{}' (scale '{})",
            block_size,
            desired_block_size,
            scale
        );
