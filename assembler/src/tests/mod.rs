...
use super::ExecutableAnonymousMemoryMap::ExecutableAnonymousMemoryMap;
use super::InstructionStreamHints::InstructionStreamHints;
use std::io::Write;
...
pub fn simple_function() {
    ...
    let _function_pointer = {
        ...
        let function_pointer: unsafe extern "C" fn() -> i32 =
            instruction_stream.nullary_function_pointer();
        ...
        assert_eq!(
            &bytes_to_string(encoded_bytes),
            "55 48 8B EC 31 C0 48 8B E5 5D C3",
            "Encoding of a basic function was wrong"
        );
...
pub fn validate_that_rust_follows_the_system_v_abi_for_bool() {
    ...
    let false_function_pointer = {
        ...
        let function_pointer: unsafe extern "C" fn() -> bool =
            instruction_stream.nullary_function_pointer();
    ...
    assert_eq!(
        unsafe { false_function_pointer() },
        false,
        "function result was not false"
    );
...
pub fn validate_that_rust_follows_the_system_v_abi_for_u128() {
    ...
    assert_eq!(
        unsafe { u128_function_pointer() },
        0xAAAAAAAA_AAAAAAAA_FFFFFFFF_FFFFFFFF,
        "function result was not as expected for u128"
    );
    ...
    assert_eq!(
        unsafe { u64_u64_function_pointer() },
        TupleWrapper {
            rax: 0xFFFFFFFF_FFFFFFFF,
            rdx: 0xAAAAAAAA_AAAAAAAA
        },
        "function result was not as expected for (u64, u64)"
    );
    ...
    assert_eq!(
        unsafe { bool_u64_function_pointer() },
        BoolU64 {
            rax: true,
            rdx: 0xAAAAAAAA_AAAAAAAA
        },
        "function result was not as expected for (bool, u64)"
    );
}
...
pub fn emit() {
    ...
    let memory = Any8BitMemory::base_64_displacement(
        Register64Bit::SystemVApplicationBinaryInterface64IntegerFunctionArgument0,
        displacement,
    );
...
pub fn simple_function2() {
    ...
    let _function_pointer = {
        ...
        let function_pointer: unsafe extern "sysv64" fn(mem: u64, offset: u64) -> i32 =
            unsafe { transmute(instruction_stream.binary_function_pointer::<i32, u64, u64>()) };

        instruction_stream.add_Any64BitMemory_Immediate32Bit(
    ...
    let mut mem = vec![1, 2, 3, 4, 5];
    let result = unsafe { _function_pointer(transmute(&mem[0] as *const i32), 2 * 4) };
    assert_eq!(mem[2], 6, "function result was not zero")
}
