...
use crate::NearJmpResult::NearJmpResult;
...
pub(crate) struct ByteEmitter {
    pub start_instruction_pointer: InstructionPointer,
    pub instruction_pointer: InstructionPointer,
    pub end_instruction_pointer: InstructionPointer,
    pub bookmark: InstructionPointer,
}
...
impl ByteEmitter {
    ...
    pub(crate) fn emit_bytes(&mut self, bytes: &[u8]) {
        ...
        debug_assert!(
            self.instruction_pointer + length <= self.end_instruction_pointer,
            "Not enough space to emit '{}' byte(s)",
            length
        );
        unsafe {
    ...
    #[inline(always)]
    pub(crate) fn emit_prefix_group2_for_segment_register(
        &mut self,
    ...
    #[inline(always)]
    pub(crate) fn emit_3_byte_vex_prefix(
        &mut self,
        r_bit: u8,
        x_bit: u8,
        b_bit: u8,
    ...
    #[inline(always)]
    pub(crate) fn insert_8_bit_effective_address_displacement(
        &mut self,
        insert_at_instruction_pointer: InstructionPointer,
        target_instruction_pointer: InstructionPointer,
    ) -> ShortJmpResult {
    ...
    #[inline(always)]
    pub(crate) fn insert_32_bit_effective_address_displacement(
        &mut self,
        insert_at_instruction_pointer: InstructionPointer,
        target_instruction_pointer: InstructionPointer,
    ) -> NearJmpResult {
    ...
    pub(crate) fn emit_u8(&mut self, emit: u8) {
        ...
        debug_assert!(
            ...
            "Not enough space to emit an u8"
        );
    ...
    pub(crate) fn skip_u8(&mut self) {
        ...
        debug_assert!(
            self.instruction_pointer + Size <= self.end_instruction_pointer,
            "Not enough space to skip an u8"
        );
    ...
    pub(crate) fn skip_u32(&mut self) {
        ...
        debug_assert!(
            self.instruction_pointer + Size <= self.end_instruction_pointer,
            "Not enough space to skip an u32"
        );
    ...
    pub(crate) fn skip_bytes(&mut self, count: usize) {
        debug_assert!(
            self.instruction_pointer + count <= self.end_instruction_pointer,
            "Not enough space to skip '{}' bytes",
            count
        );
        if cfg!(debug_assertions) {
            for _ in 0..count {
